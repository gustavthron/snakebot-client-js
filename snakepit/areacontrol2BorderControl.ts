...
function getAvailableSpace(
  ...
  floodfillLimit = FLOODFILL_LIMIT,
): { availableSpace: number; trapped: boolean } {
  if (head === undefined) {
    return { availableSpace: 0, trapped: true };
  }
...
function heuristic(gameMap: GameMap, playerId: string, opponentId: string): number {
  ...
  const playerAvailableSpace = oldGetAvailableSpace(gameMap, playerHead);
  const opponentAvailableSpace = oldGetAvailableSpace(gameMap, opponentHead);
...
export async function getNextMove(gameMap: GameMap) {
  ...
  let fullyTrapped = true;
  const scores = [];
  for (const move of bestMoves) {
    const possibleRemovedTail = applyMove(gameMap, move, playerId);
    const head = gameMap.playerSnake.headCoordinate;
    const { availableSpace: score, trapped: trapped } = getAvailableSpace(
      gameMap,
      head,
      gameMap.playerId,
      floodFillMap,
    ...
    const xCoord = gameMap.playerSnake.coordinates[0].x;
    const yCoord = gameMap.playerSnake.coordinates[0].y;
  ...
  if (fullyTrapped) {
    ...
    if (movesAwayFromBorder.length > 0) {
      ...
      if (movesAwayFromBorder[0].score >= 30 || (bestScore - movesAwayFromBorder[0].score < 5 && bestScore > 5)) {
        okayMove = movesAwayFromBorder[0].move;
      }
    }
  }
