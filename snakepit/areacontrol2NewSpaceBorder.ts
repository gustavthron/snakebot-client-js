...
function heuristic(gameMap: GameMap, playerId: string, opponentId: string): number {
  ...
  let { availableSpace: opponentAvailableSpace, trapped: trappedO } = getAvailableSpace(
    gameMap,
    opponentHead,
    opponentId,
    floodFillMap,
  ...
  const { availableSpace: playerAvailableSpace, trapped: trapped } = getAvailableSpace(
    gameMap,
    playerHead,
    playerId,
    floodFillMap,
...
export async function getNextMove(gameMap: GameMap) {
  ...
  if (fullyTrapped) {
    ...
    if (movesAwayFromBorder.length > 0) {
      ...
      if (movesAwayFromBorder[0].score / bestScore >= 0.3 && bestScore > 3) {
