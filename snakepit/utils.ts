...
function getOthersTailCoordinates(gameMap: GameMap): Set<_Coordinate> {
  ...
  for (const [id, snake] of gameMap.snakes) {
    if (
      id === gameMap.playerId ||
      snake.tailCoordinate === undefined ||
...
export function getTile(gameMap: GameMap, coordinate: Coordinate): Tile {
  ...
  try {
    ...
    if (tileType === TileType.Snake && getOthersHeadCoordinates(gameMap).has(_coordinate)) {
  ...
  let walkable =
    walkableTileTypes.has(tileType) ||
    (tileType === TileType.Snake && getOthersTailCoordinates(gameMap).has(_coordinate));
...
interface SearchCache {
  gameTick: number;
  searches: Map<number, SearchCacheData>;
  count: number;
}
...
function popSearchCache(position: number): SearchCacheData | undefined {
  let cacheKey = -1;
  let res;
  for (const [key, value] of searchCache.searches.entries()) {
    if (value.found.has(position)) {
      cacheKey = key;
      res = value;
      break;
    }
  }
  searchCache.searches.delete(cacheKey);
  return res;
}

export function search(p: SearchParameters): SearchResult {
  if (p.gameMap.gameTick !== searchCache.gameTick) {
    searchCache.gameTick = p.gameMap.gameTick;
    searchCache.searches = new Map();
    searchCache.count = -1;
  }
  searchCache.count++;
  const from = getTile(p.gameMap, p.from);
  const to = getTile(p.gameMap, p.to);
  const fromCache = popSearchCache(from.position);
  let found = fromCache?.found ?? new Set([from.position]);
  const open = fromCache?.open ?? [from];
  let walkableSize = fromCache?.walkableSize ?? (from.walkable ? 1 : 0);
  while (open.length > 0 && !found.has(to.position)) {
    const node = open.shift();
    if (!node) continue;
    for (const edge of getEdgeTiles(p.gameMap, node.coordinate)) {
      if (!found.has(edge.position)) {
        const edgeCache = popSearchCache(edge.position);
        if (edgeCache) {
          found = new Set([...found, ...edgeCache.found]);
          open.push(...edgeCache.open);
          walkableSize += edgeCache.walkableSize;
        } else {
          found.add(edge.position);
          if (edge.walkable) {
            walkableSize++;
            open.push(edge);
          }
  ...
  searchCache.searches.set(searchCache.count, { found, open, walkableSize });
  return { hasPath: found.has(to.position), walkableSize };
}
