...
export async function getNextMove(gameMap: GameMap) {
  ...
  if (enemySnakes[0].id === playerSnake.id || enemySnakes[1].id === playerSnake.id) {
    ...
    if (aliveEnemySnakes.length <= 2) {
      ...
      return getNextMoveMini(gameMap);
    }
  }
  console.log('Area Snake');
  return getNextMoveArea(gameMap);
}

export function getNextMoveArea(gameMap: GameMap) {
  ...
  //   const possibleRemovedTail = applyMove(gameMap, move, playerId);
  //   let availableSpace = getAvailableSpace(gameMap, gameMap.playerSnake.headCoordinate, playerId, FLOODFILL_DEPTH);
  //   undoMove(gameMap, possibleRemovedTail, playerId);
